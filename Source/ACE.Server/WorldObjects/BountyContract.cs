...
namespace ACE.Server.WorldObjects
{
    public partial class BountyContract : WorldObject
    {
        ...
        public bool IsActive => State == BountyState.Active;
        public bool IsCompleted => State == BountyState.Completed;
        public bool IsExpiredState => State == BountyState.Expired;
        public bool IsPending => State == BountyState.Pending;
        ...
        {
            get => (BountyState)BountyContractState;
            set => BountyContractState = (int)value;
        }

        public void SetState(BountyState newState, Player owner = null)
        {
            if (State == newState)
                return;

            if (State == BountyState.Expired)
                return;

            if (!IsValidTransition(State, newState))
            {
                var message = $"Invalid bounty state transition: {State} → {newState} for contract {Name} ({Guid.Full})";
        ...
        private bool IsValidTransition(BountyState from, BountyState to)
        {
            return (from, to) switch
            {
                (BountyState.Pending, BountyState.Active) => true,
                (BountyState.Active, BountyState.Completed) => true,
                (BountyState.Active, BountyState.Expired) => true,
                (BountyState.Completed, BountyState.Expired) => true,
                _ => false
            };
        }
        ...
        public string BuildBountyContractLongDescription()
        {
            try
            {
                ...
                {
                    longDesc += $"This target is currently on a kill streak of {targetKillStreak}. This means they have killed {targetKillStreak} players without dying. Bounty targets on kill streaks are often more dangerous and may be worth more to hunt down.\n\n";
                }
