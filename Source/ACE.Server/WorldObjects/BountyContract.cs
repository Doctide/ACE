    ...
    public partial class BountyContract : WorldObject
    {
        ...
        public void UpdateUiEffects(Player owner)
        {
            if (IsBountyExpired)
            {
                ...
                owner?.SendBountyMessage($"{Name} has expired. Turn it in to a Bounty Collector to possibly receive some compensation.");
                return;
            }
        ...
        public string BuildBountyContractLongDescription()
        {
            try
            {
                ...
                var bountyLocationStringAmount = bountyLocationCurrencyWeenie.BuildAmountString((uint)bountyLocationRewardAmount);

                if (highPriority)
                {
                    var priorityOwnerName = bountyTarget.GetProperty(ACE.Entity.Enum.Properties.PropertyString.BountyPriorityOwnerName);
                    longDesc += $"This contract is a high priority target assigned by {priorityOwnerName}. They are rewarding {bountyLocationStringAmount} for completing this contract.\n\n";
