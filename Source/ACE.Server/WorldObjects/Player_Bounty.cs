    ...
    partial class Player
    {
        ...
        private bool CheckBountyContractTurnIns(WorldObject npc, WorldObject item)
        {
            ...
            try
            {
                ...
                {
                    var rewardedAmount = wopCurrencyWeenie.BuildAmountString(rewardAmount.Value);
                    SendDelayedNpcResponse(npc, $"This bounty was a high priorty target, you have been rewarded {rewardedAmount}.",
                        () => GiveFromEmote(npc, BountyContract.BountyWopCurrencyWcid, amount: rewardAmount.Value));

                    PlayerManager.BroadcastToAll(new GameMessageSystemChat($"{Name} has completed a bounty contract for high priority target {bountyName} and received {rewardedAmount}!", ChatMessageType.WorldBroadcast));
                }
