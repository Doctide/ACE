...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
        ...
        public void BountyTick()
        {
            ...
            {
                if (bounty.IsBountyExpired)
                {
                    ActiveBounties.Remove((uint)bounty.BountyTargetGuid.Value);
                }
            }
        }
            ...
            try
            {
                if (npc.WeenieClassId == BountyContract.BountyNPCWcid && item is BountyContract contract)
                {
                    ...
                    if (contract.IsBountyExpired)
                    {
                        ...
                        var phialOfBloodyTears = WorldObjectFactory.CreateNewWorldObject(1000003);
                        TryCreateInInventoryWithNetworking(phialOfBloodyTears);
                        return BountyResult.Ignore;
                    }
        ...
        public BountyResult CheckBountyPurchase(WorldObject npc, WorldObject item)
        {
            ...
            try
            {
                if (npc.WeenieClassId == BountyContract.BountyNPCWcid && item.WeenieClassId == BountyContract.BountyPurchaseTokenWcid)
                {
                    ...
                    {
                        SendDelayedMessage($"{npc.Name} tells you, \"You have reached the maximum number of active bounties. Please complete or turn in your current bounties before purchasing new ones.\"", ChatMessageType.Tell);
                        return BountyResult.Failure;
                    }
